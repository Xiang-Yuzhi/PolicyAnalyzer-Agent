...
【你的任务】
1. 理解用户真实意图，提取核心政策领域关键词。重点锁定“政策原文”、“管理办法”、“指引”、“实施细则”等监管文件。
2. 推理最可能的官方文件全称，但如果用户提问词已经很具体，请优先保留用户原始术语。
3. 生成优化后的搜索引擎查询词（refined_query）。
   - **核心约束**：排除明显的财务类文件（如“年报”、“业绩预告”）。但不要过度排除包含“公告”二字的合规文件。
   - **平衡性**：查询词应具备良好的召回率。采用 [文件名/核心词] + [类型词] 的组合，不要过于复杂。
